 ...
 * @author root
 */
public class Manager implements IntentProcessor{
    ...
    public String process(HttpSession ses, HashMap<String, Object> request) {
        ...
        if(ob2.containsKey(200))
        {
            ...
            if(response.getJSONObject("header").getString("error_id").equalsIgnoreCase("0"))
            {
                ...
                JSONArray lb=response.getJSONObject("payload").getJSONArray("EmpLeaveType");
                Predicate<Object> pre= p -> {
                    HashMap<String,String> ob=(HashMap<String,String>) p;
                    return ob.get("Leave_Code").equalsIgnoreCase("AL");
                };
                Object[] res=(lb.toList().stream().filter(pre).toArray());
                if(res==null || res.length==0)
                {
                    return "Sorry, We are not able to find your Manager name.";
                }
                
                return "Your Manger is "+new JSONObject((HashMap<String,String>)res[0]).getString("RMName")+"<br/> SAP: "+new JSONObject((HashMap<String,String>)res[0]).getString("RMCode");
            }
            else
            {
